...
class SlideimageController extends Controller
{
	...
	public function actionAdd()
	{
		...
		if(isset($_POST['Slideimage']))
		{
			...
			if($model->save()) {	
				...
				if (!empty($myUpload)) {
					$this->_mapPath .= '/Gambar'.$model->Slideimage;
			
					$myUpload->saveAs($this->_mapPath .'/Gambar.' . $myUpload->getExtensionName());
				}	

				$this->redirect(array('view','id'=>$model->id_gbr));
	...
	{
		$model=$this->loadModel($id);
			$tmpNamaUnitKerja = $model->judul_gambar;
	...
	{
		$dataProvider=new CActiveDataProvider('Slideimage');
	...
	{
		$model=new Slideimage('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['judul']))
			$model->attributes=$_GET['judul'];
