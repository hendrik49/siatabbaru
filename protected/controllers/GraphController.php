...
class GraphController extends Controller
{
	...
	function forceDownload($filename = '', $data = '')
	{
		...
		// Generate the server Graphs
		if (strpos($_SERVER['HTTP_USER_AGENT'], "MSIE") !== FALSE)
		{
			Graph('Content-Type: "'.$mime.'"');
			Graph('Content-Disposition: attachment; filename="'.$filename.'"');
			Graph('Expires: 0');
			Graph('Cache-Control: must-revalidate, post-check=0, pre-check=0');
			Graph("Content-Transfer-Encoding: binary");
			Graph('Pragma: public');
			Graph("Content-Length: ".strlen($data));
		}
		else
		{
			Graph('Content-Type: "'.$mime.'"');
	...
	{
		$model=new Graph('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Graph']))
			$model->attributes=$_GET['Graph'];
		...
		$dataProvider=new CActiveDataProvider('Graph', array(
			'criteria'=>$criteria,
			'sort'=>array(
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='Graph-form')
		{
