...
class TeknisGaSumur extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('tahun_bangun, rehab, rencana_rehab, nama_lembaga, jumlah_anggota,legalitas, tahun_berdiri, keaktifan, no_kontrak, status_kelola, status_operasi, keterangan', 'length', 'max'=>100),
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_sumur5 WHERE ID_IDBalaiMa='$id'");
