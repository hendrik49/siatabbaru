...
class TeknisMA extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'mataair'=>array(self::BELONGS_TO, 'MataAir', 'ID_IDBalaiGa'),
			'manfaat'=>array(self::BELONGS_TO, 'ManfaatMA', 'ID_IDBalaiGa'),
			'teknissat'=>array(self::BELONGS_TO, 'TeknisMA', 'ID_IDBalaiGa'),
		);
	}
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_mataair3 WHERE ID_IDBalaiGa='$id'");
