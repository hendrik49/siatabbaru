...
class TeknisWaTampungan extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'tampungan'=>array(self::BELONGS_TO, 'Tampungan', 'ID_IDBalaiPat'),
			'manfaat'=>array(self::BELONGS_TO, 'ManfaatTampungan', 'ID_IDBalaiPat'),
			'teknissat'=>array(self::BELONGS_TO, 'TeknisTampungan', 'ID_IDBalaiPat'),
		);
	}
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_tampungan4 WHERE ID_IDBalaiPat='$id'");
