...
class TeknisGaMA extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'mataair'=>array(self::BELONGS_TO, 'MataAir', 'ID_IDBalaiMa'),
			'manfaat'=>array(self::BELONGS_TO, 'ManfaatMA', 'ID_IDBalaiMa'),
			'teknissat'=>array(self::BELONGS_TO, 'TeknisMA', 'ID_IDBalaiMa'),
			'teknisdua'=>array(self::BELONGS_TO, 'TeknisWaMA', 'ID_IDBalaiMa'),
		);
	}
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_mataair5 WHERE ID_IDBalaiMa='$id'");
