...
class TeknisWaMA extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'mataair'=>array(self::BELONGS_TO, 'MataAir', 'ID_IDBalaiPat'),
			'manfaat'=>array(self::BELONGS_TO, 'ManfaatMA', 'ID_IDBalaiPat'),
			'teknissat'=>array(self::BELONGS_TO, 'TeknisMA', 'ID_IDBalaiPat'),
		);
	}
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_mataair4 WHERE ID_IDBalaiPat='$id'");
