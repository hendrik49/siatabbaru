...
class TeknisGaTampungan extends CActiveRecord
{
	/**
	...
	public function relations()
	{
		...
		return array(
			...
			'tampungan'=>array(self::BELONGS_TO, 'Tampungan', 'ID_IDBalaiMa'),
			'manfaat'=>array(self::BELONGS_TO, 'ManfaatTampungan', 'ID_IDBalaiMa'),
			'teknissat'=>array(self::BELONGS_TO, 'TeknisTampungan', 'ID_IDBalaiMa'),
			'teknisdua'=>array(self::BELONGS_TO, 'TeknisWaTampungan', 'ID_IDBalaiMa'),
		);
	}
	...
	public static function getNoDataByAdmin()
	{
		if (isset(Yii::app()->user->uid)) {
			$id = Yii::app()->user->uid;
			$command = Yii::app()->db->createCommand("SELECT * FROM t_tampungan5 WHERE ID_IDBalaiMa='$id'");
